...
public class DistanceTest {
	...
	public static void runAccuracyTest(List<Gift> gifts, int testCnt) {
		...
		for (int i=0; i< testCnt-1; i++) {
			...
			double exactDist = GiftPosition.getHaversineDistance(gifts.get(giftIds1).getPosition(), gifts.get(giftIds2).getPosition());
			double approxDist =  GiftPosition.approximateHaversineDistance(gifts.get(giftIds1).getPosition(), gifts.get(giftIds2).getPosition());
	...
	public static void runTimeTest(List<Gift> gifts, int testCnt) {
		...
		for (Map.Entry<GiftPosition, GiftPosition> entry: testGifts.entrySet()) {	
			GiftPosition.getHaversineDistance(entry.getKey(), entry.getValue());		
		}
		...
		for (Map.Entry<GiftPosition, GiftPosition> entry: testGifts.entrySet()) {
			GiftPosition.approximateHaversineDistance(entry.getKey(), entry.getValue());
		}
